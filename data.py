...
class AFM(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if self.train:
            ...
            if self.num_train_per_cls != -1:  # Will return same split of data if num_train_per_cls is same!
                ...
                self.num_train_per_cls = int(
                    len(self.DF.targets) / len(self.DF.classes)
                )  # updates -1 to true num_train_per_cls

            # If wanted to add unlabeled training data:
            if unlabeled_percent > 0:
                # Further trim TRAIN data to acommodate unlabeled_percent
                num_labeled_needed_per_cls = int(
                    self.num_train_per_cls * (1 - unlabeled_percent)
                )  # Num labeled data if w/ unlabeled data
        ...
        for data, targets in loader:  # Single loop only since batch_size = entire dataset
            self.data = data
            self.data = torch.squeeze(
                self.data
            ).numpy()  # keep as np.ndarray, following cifar10 data import standards AND for data processing later
        ...
        if self.train and unlabeled_percent > 0:  # Update: Should be moved to above as commented
            ...
            self.replace_w_unlabeled_data(
                unlabeled_percent, num_labeled_needed_per_cls
            )  # Fulfils unlabeled % while keeping constant total num train sample (By removing labeled data with unlabeled)
    ...
    def trim_data(self, sample_per_cls):
        ...
        sss = StratifiedShuffleSplit(
            n_splits=1, train_size=sample_per_cls * len(self.DF.classes), random_state=self.seed
        )
    ...
    def add_unlabeled_data(self, unlabeled_percent):
        ...
        labeled_sample_per_cls = c2[
            0
        ]  # Usiang first class num samples as reference, assuming all class have equal num samples
        ...
        if needed_sample > min_sample:  # If more unlabeled sample needed than available
            ...
            print(
                f"Removing {num_reduce} matching labels data per class to fulfil the {unlabeled_percent} unlabeled_percent!"
            )
            print("=========================================")
    ...
    def replace_w_unlabeled_data(self, unlabeled_percent, num_labeled_needed_per_cls):
        ...
        labeled_sample_per_cls = c2[
            0
        ]  # Using first class num samples as reference, assuming all class have equal num samples
        ...
        # ================================================================
            ...
            if self.mode.startswith("3class"):
                # Combine class 0 and 3 (~ half and half)
                paths = (
                    all_paths[0][: int(needed_sample / 2)] + all_paths[3][: int(needed_sample - int(needed_sample / 2))]
                )
                # Load class 0 and 3
                for p in paths:
                    unlabeled_data.append(np.loadtxt(p, delimiter=","))  # check dimension
