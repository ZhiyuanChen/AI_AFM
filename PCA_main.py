...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ns", "--num_sym", type=int, default=0, help="Augmentation: Symbolize Data (default=0, no symbolize)"
    )
    ...
    parser.add_argument(
        "-tar", "--tar_name", type=str, required=True, help="Specify model weight (without .tar) to be loaded"
    )
    parser.add_argument(
        "-clf",
        "--classifier_model",
        type=str,
        default="FSclf1",
        choices=["FSclf1", "FSclf2"],
        help="Choices of models (default='FSclf1')",
    )
    ...
    loader = torch.utils.data.DataLoader(
        AFM_train.DF, batch_size=len(AFM_train.DF.samples)
    ...
    loader = torch.utils.data.DataLoader(
        AFM_test.DF, batch_size=len(AFM_test.DF.samples)
    )  # batch_size = entire dataset
    for data, targets in loader:  # Single loop only since batch_size = entire dataset
