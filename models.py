...
class ToyNetS(FewShotNet):
    def __init__(self, input_dim, out_dim):
        super().__init__(input_dim, out_dim)
        self.enc = nn.Sequential(
            nn.Linear(input_dim, 200), nn.ReLU(), nn.Linear(200, 100), nn.ReLU(), nn.Linear(100, out_dim)
        )
...
class ToyNetS2(FewShotNet):
    def __init__(self, input_dim, out_dim):
        super().__init__(input_dim, out_dim)
        self.enc = nn.Sequential(nn.Linear(input_dim, 200), nn.Linear(200, 100), nn.Linear(100, out_dim))
...
class ToyNetS3(FewShotNet):
    def __init__(self, input_dim, out_dim):
        super().__init__(input_dim, out_dim)
        self.enc = nn.Sequential(
            nn.Linear(input_dim, 256), nn.ReLU(), nn.Linear(256, 128), nn.ReLU(), nn.Linear(128, out_dim)
        )
...
class ToyNetXS(FewShotNet):
    def __init__(self, input_dim, out_dim):
        super().__init__(input_dim, out_dim)
        self.enc = nn.Sequential(nn.Linear(input_dim, 200), nn.ReLU(), nn.Linear(200, out_dim))
...
class CerberusNet(FewShotNet):
    def __init__(self, input_dim, out_dim):
        super().__init__(input_dim, out_dim)
        self.enc = nn.Sequential(nn.Linear(input_dim, 256), nn.Linear(256, 128), nn.Linear(128, out_dim))
...
class CerberusNetL(FewShotNet):
    def __init__(self, input_dim, out_dim):
        super().__init__(input_dim, out_dim)
        self.enc = nn.Sequential(
            nn.Linear(input_dim, 256), nn.Linear(256, 128), nn.Linear(128, 64), nn.Linear(64, out_dim)
        )
...
class Convo1DNet(FewShotNet):
    ...
    def __init__(self, input_dim=1, out_dim=10):
        ...
        self.enc = nn.Sequential(
            ...
            nn.Flatten(),
            nn.LazyLinear(512),
            nn.PReLU(),  # LazyLinear infers the input dim
            nn.Linear(512, 256),
            nn.PReLU(),
            nn.Linear(256, 128),
            nn.PReLU(),
...
class MNISTNet(nn.Module):
    def __init__(self, input_dim=1, out_dim=2):
        ...
        self.fc = nn.Sequential(
            nn.Linear(64 * 4 * 4, 256), nn.PReLU(), nn.Linear(256, 256), nn.PReLU(), nn.Linear(256, out_dim)
        )
...
model_list = {
    ...
    "cerb": CerberusNet,
    "cerbL": CerberusNetL,
    "cerbXL": CerberusNetXL,
    "convo1D": Convo1DNet,
    "convo1DS": Convo1DNetS,
    "convo1DDrp": Convo1DNetDrp,
    "convo1DDrp2": Convo1DNetDrp2,
    "convo1DS2": Convo1DNetS2,
    "convo1D2": Convo1DNet2,
    "convo1D3": Convo1DNet3,
    "FSclf1": FewShotClf_1,
    "FSclf2": FewShotClf_2,
}
...
def LoadModel(args):
    ...
    if args.model_type == "siamese":
        model = (
            SiameseNet(model_list[args.model](input_dim=args.sample_dim, out_dim=args.model_out_dim))
            .to(args.device)
            .double()
        )
        ...
        model = (
            TripletNet(model_list[args.model](input_dim=args.sample_dim, out_dim=args.model_out_dim))
            .to(args.device)
            .double()
        )
...
def FewShotClf_LoadModel(args):
    ...
    trained_model = (
        model_list[args.model](input_dim=args.sample_dim, out_dim=args.model_out_dim).to(args.device).double()
    )
    ...
    model = (
        model_list[args.classifier_model](input_dim=args.sample_dim, out_dim=int(args.data[0])).to(args.device).double()
    )
