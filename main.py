...
from trainer import fit, logger, save_loss_plot
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="2class",
        choices=["3class_matching", "2class_s_vs_r", "2class_n_vs_r"],
        help="Choices of dataset (default='2class')",
    )
    parser.add_argument(
        "-opt",
        "--opt",
        ...
        help="Choices of optimizer (default='sgd')",
    )
    parser.add_argument(
        "-v",
        "--verbosity",
        type=int,
        default=1,
        help="Verbosity. 0: No output, 1: Epoch-level output, 2: Batch-level output",
    )
    ...
    parser.add_argument(
        ...
        help="Choices of models (default='toy')",
    )
    parser.add_argument(
        "-mt",
        "--model_type",
        ...
        help="Model type (default='siamese')",
    )
    ...
    parser.add_argument(
        "-log", "--log_interval", type=int, default=10, help="Saving model at every specified interval (default = 10)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Percent of unlabeled data with range of [0.0,1.0) (default=0.0)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Number of training samples per class (default=-1, all data)",
    )
    ...
    parser.add_argument(
        ...
        default="minmax",
        help="Scale data with minmax(default), normalization or no scaling",
    )
    parser.add_argument(
        "-pp",
        "--suppress_preprocess",
        action="store_true",
        default=False,
        help="Augmentation: Suppress Data Processing Block",
    )

    parser.add_argument(
        "--use_cuda", action="store_true", default=True, help="Use CUDA if available"
    )  # Conflicting: Always True
    parser.add_argument(
        "-sche_step", "--LR_sche_step", type=int, default=1, help="Stepsize for LR scheduler"
    )  # For StepLR
    parser.add_argument(
        "-sche_gamma", "--LR_sche_gamma", type=float, default=1.0, help="Gamma for LR scheduler"
    )  # For StepLR

    parser.add_argument(
        "-tar", "--tar_name", type=str, default="", help="Specify model weight (without .tar) to be loaded"
    )
    ...
    AFM_train = AFM(
        **AFM_kwargs, train=True, unlabeled_percent=args.unlabeled_percent, num_train_per_cls=args.num_train_per_cls
    )
    ...
    train_loader = torch.utils.data.DataLoader(
        fewshot_AFM_train, batch_size=args.batch_size, num_workers=1, shuffle=True
    )  # , sampler=train_sampler)
    val_loader = torch.utils.data.DataLoader(
        fewshot_AFM_test, batch_size=args.batch_size, num_workers=2
    ...
    if args.model in ["convo1D", "convo1DS", "convo1DDrp", "convo1DDrp2", "convo1DS2", "convo1D2", "convo1D3"]:
        ...
        sample_data = [
            torch.rand(args.batch_size, args.sample_dim).to(args.device).double() for _ in range(num_x_needed)
        ]
        _ = model(*sample_data)
