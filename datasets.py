        ...
        if not self.train:
            ...
            negative_pairs = [
                [
                    i,
                    random_state.choice(
                        self.label_to_indices[np.random.choice(list(self.labels_set - set([self.labels[i]])))]
                    ),
...
class SiameseEM(Dataset):
    ...
    def __init__(self, labeled_dataset, unlabeled_dataset):
        ...
        self.labeled_labels = np.array(
            [str(t) for t in self.labeled_dataset.targets]
        )  # str version of self.labeled_dataset.targets
        self.unlabeled_labels = np.array(
            [str(t) for t in self.unlabeled_dataset.targets]
        )  # str version of self.unlabeled_dataset.targets
        ...
        self.unlabeled_label_to_indices = {
            label: np.where(self.unlabeled_labels == label)[0] for label in self.labels_set
        }

        # Convertions for faster/easier __getitem__()
        self.labeled_dataset.data = [
            torch.tensor(d) for d in self.labeled_dataset.data
        ]  # ndarray(ndarray) --> list(tensor)
        self.unlabeled_dataset.data = [
            torch.tensor(d) for d in self.unlabeled_dataset.data
        ]  # ndarray(ndarray) --> list(tensor)

        # CONVERT TO NDARRAY!!!
        self.labeled_dataset.targets = np.array(
            [int(d) for d in self.labeled_dataset.targets]
        )  # list(np.int64) --> ndarray(int)
        self.unlabeled_dataset.targets = np.array(
            [int(d) for d in self.unlabeled_dataset.targets]
        )  # list(np.int64) --> ndarray(int)
    ...
    def __getitem__(
        self, index
    ):  # Returns [(labeled_1, labeled_2, ...), (unlabeled_1, unlabeled_2, ...)], [target_1, target_2, ...]
        bin_targets = np.ones(len(self.labeled_dataset.targets), dtype=int)  # First create all one targets
        bin_targets[
            self.labeled_dataset.targets == self.unlabeled_dataset.targets[index]
        ] = 0  # Replace similar target/class to zero

        # Return 'data' in single tensor
        return (
            (
                torch.stack(self.labeled_dataset.data),
                torch.stack([self.unlabeled_dataset.data[index]] * len(self.labeled_dataset.targets)),
            ),
            bin_targets,
            self.labeled_dataset.targets,
            self.unlabeled_dataset.targets[index],
        )
...
class SiameseMNIST(Dataset):
    ...
    def __init__(self, dataset):
        ...
        if not self.train:
            ...
            positive_pairs = [
                [i, random_state.choice(self.label_to_indices[self.labels[i]]), 1] for i in range(0, len(self.data), 2)
            ]
