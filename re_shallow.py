...
from util import knn_, rf_, svm_
...
if mode == "trainsz":
    ...
    for ts in train_size:  # Loop train sizes dim
        ...
        for idx in range(n_fold):  # Loop training index, for each n_folds
            ...
            if algo == "RF":
                acc_, best_depth, best_num_trees, seed = rf_(
                    depthrange, numtreerange, _X_train, _Y_train, X_test, Y_test
                )
                acc.append(acc_)

            if algo == "SVM":
                acc_, best_C, best_kernel, best_degree, best_gamma, best_df_shape, seed = svm_(
                    Cvals, kernels, degrees, gamma_vals, df_shapes, _X_train, _Y_train, X_test, Y_test
                )
                acc.append(acc_)
    ...
    ax.fill_between(t, (mean_acc - std_acc) * 100, (mean_acc + std_acc) * 100, color="b", alpha=0.1)
